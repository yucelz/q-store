...
class IonQQuantumBackend:
    ...
    def execute_circuit(self, circuit: cirq.Circuit, repetitions: int = 1000) -> CircuitResult:
        """
        ...
        """
        # Submit job to IonQ
        job = self.service.run(circuit=circuit, repetitions=repetitions, name="q_store_query")
    ...
    def create_entangled_state(self, vectors: List[np.ndarray]) -> cirq.Circuit:
        ...
        for i, vector in enumerate(vectors):
            ...
            qubit_map = {cirq.LineQubit(j): sub_qubits[j] for j in range(len(sub_qubits))}
        ...
        for i in range(n_vectors - 1):
            circuit.append(
                cirq.CNOT(qubits[i * qubits_per_vector], qubits[(i + 1) * qubits_per_vector])
            )
    ...
    def measure_in_basis(
        self, circuit: cirq.Circuit, basis: str = "computational"
    ) -> CircuitResult:
        """
    ...
    def _decompose_to_native_gates(
        self, state: np.ndarray, qubits: List[cirq.Qid]
    ) -> List[cirq.Operation]:
        """
    ...
    def _process_results(self, results, total_shots: int) -> CircuitResult:
        ...
        return CircuitResult(counts=counts, total_shots=total_shots, probabilities=probabilities)
